...
- 不强制“一文件一类型”；同一业务主题的输入、结果和小型私有类型可以与拥有它们的函数或方法放在同一文件。
...
- 不需要持有依赖、配置或跨调用状态的业务操作优先使用普通函数，不为“以后可能扩展”先包装成结构体。
- 结构体只在需要持有真实依赖、配置、运行状态或一组共享不变量时创建；不要用空结构体包装原本独立的普通函数。
- 构造函数只负责注入必需依赖、校验创建时不变量或建立必要默认值；类型零值可直接使用时，不机械增加 `NewXxx`。
...
- Provider 因 Dever 注册协议需要导出接收者；只承载真实 `ProviderXxx` 适配方法时，可以使用没有字段的接收者类型，这不是普通业务结构体的默认写法。
- 导出方法表达需要被其它包调用的业务用例或稳定协议；仅服务于当前实现的步骤使用小写私有函数或方法。
